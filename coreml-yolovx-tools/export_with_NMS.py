...
nc = 4
classLabels = [f"label{i}" for i in range(nc)]
...
def createNmsModelSpec(nnSpec, opt):
    '''
    ...
    nms.iouThreshold = opt.iou_thres
    nms.confidenceThreshold = opt.conf_thres
...
def main():
    ...
    parser.add_argument('--model-input-path', type=str, dest="model_input_path",
                        default='best.pt', help='path to yolov5 model')
    parser.add_argument('--model-output-directory', type=str,
                        dest="model_output_directory", default='data', help='model output path')
    parser.add_argument('--model-output-name', type=str, dest="model_output_name",
                        default='yolov7_ios', help='model output name')
    ...
    sampleInput = torch.zeros((1, 3, SIZE, SIZE))
    checkInputs = [(torch.rand(1, 3, SIZE, SIZE),),
    ...
    # Create nms logic
    nmsSpec = createNmsModelSpec(builder.spec, opt)
